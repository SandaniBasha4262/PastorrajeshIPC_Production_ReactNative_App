...
const MusicPlayer = ({ navigation }) => {
  ...
  const audioPlayer = useMemo(() => createAudioPlayer(null), []);
  ...
  useEffect(() => {
    ...
    return () => {
      try {
        ...
        console.error("Error removing audioPlayer:", error);
      }
    };
  }, [audioPlayer]);
  ...
  const playTrack = async (track, index) => {
    ...
    const targetIndex = typeof index === "number" ? index : currentTrackIndex;
    setCurrentTrackIndex(targetIndex);

    try {
      audioPlayer.replace(track.audio_url);
      audioPlayer.play();
      updatePlayingSongTitle(track.tittle);
    } catch (error) {
  ...
  const handleSliderChange = (value) => {
    if (audioPlayer) {
      audioPlayer.seekTo(value * (duration || 0));
    }
  };
    ...
    if (confirmed) {
      if (audioPlayer) {
        try {
          ...
          console.error("Error pausing audioPlayer:", error);
        }
      }
      navigation.goBack();
    }
  ...
  const nextTrack = () => {
    if (currentTrackIndex < tracks.length - 1) {
      ...
      playTrack(nextTrackItem, nextIndex);
    }
  };
  ...
  const prevTrack = () => {
    if (currentTrackIndex > 0) {
      ...
      playTrack(prevTrackItem, prevIndex);
    }
  };
              ...
              <>
                ...
                <View style={styles.seekBarContainer}>
                  <Slider
                      ...
                      value={duration > 0 ? (position || 0) / duration : 0}
                      //value={undefined}
                      minimumValue={0}
                      maximumValue={1}
                      step={0.01}
